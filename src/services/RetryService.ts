...
export interface RetryOptions {
  ...
  circuitBreaker?: CircuitBreaker; // For testing purposes
}
...
export class CircuitBreaker {
  ...
  public __forceStateForTesting(state: CircuitBreakerState): void {
    this.state = state;
...
export class RetryService {
  private readonly defaultOptions: Required<
    Omit<RetryOptions, "circuitBreaker">
  >;
  ...
  constructor(defaultOptions: RetryOptions = {}) {
    this.defaultOptions = {
      ...
      retryableErrors:
        defaultOptions.retryableErrors ?? this.defaultRetryableErrorCheck,
  ...
  public async executeWithRetry<T>(
    ...
    while (attempt < mergedOptions.maxAttempts) {
      this.stats.totalAttempts++; // Count each actual attempt
      attempt++;
      try {
        if (circuitBreaker) {
        ...
        const baseDelay = Math.min(
          mergedOptions.baseDelayMs *
            Math.pow(mergedOptions.backoffFactor, attempt - 1),
        ...
        this.stats.averageRetryDelay =
          this.totalRetryDelay / this.stats.totalRetries;

        // Call retry callback (wrap in try-catch to handle errors gracefully)
        try {
          mergedOptions.onRetry(attempt, lastError, delayMs);
        } catch (callbackError) {
          // Log but don't fail the retry if callback throws
  ...
  public static retryOnRateLimit(
    maxAttempts = 3,
    baseDelayMs = 5000,
  ): RetryOptions {
    return {
  ...
  public static retryOnNetworkError(
    maxAttempts = 3,
    baseDelayMs = 1000,
  ): RetryOptions {
    return {
  ...
  private mergeOptions(options: RetryOptions): Required<
    Omit<RetryOptions, "circuitBreaker">
  > & {
    ...
    return {
      ...
      retryableErrors:
        options.retryableErrors ?? this.defaultOptions.retryableErrors,
