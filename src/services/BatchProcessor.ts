...
export class BatchProcessor {
            ...
            for (const item of batch) {
              ...
              try {
                ...
                const batchError = new Error(
                  `Batch ${batchIndex} item error: ${(error as Error).message}`,
                );
  ...
  public getActiveProcessingTasks(): string[] {
    return Array.from(this.cancellationTokens.keys()).filter(
      (id) => !this.cancellationTokens.get(id),
    );
  }
