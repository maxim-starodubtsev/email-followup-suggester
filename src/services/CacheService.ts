...
export class CacheService implements ICacheService {
  ...
  public set<T>(key: string, value: T, ttl?: number): void {
    ...
    const hash = this.options.enableContentHashing
  ...
  public importCache(
    data: Array<{ key: string; entry: CacheEntry<any> }>,
  ): void {
  ...
  private evictByMemoryPressure(requiredSpace: number): CacheEvictionResult {
    ...
    for (const [key, entry] of sortedEntries) {
      if (
        this.getStats().totalMemoryUsage - freedMemory <= targetMemory &&
      ...
      case "object":
        ...
        try {
          if (Array.isArray(obj)) {
            return obj.reduce(
              (size, item) => size + this.estimateSize(item),
              0,
            );
          }
  ...
  private estimateSizeRecursive(obj: any, visited: WeakSet<object>): number {
    ...
    try {
      if (Array.isArray(obj)) {
        return obj.reduce(
          (size, item) => size + this.estimateSizeRecursive(item, visited),
          0,
        );
      }
      ...
      return Object.keys(obj).reduce((size, key) => {
        return (
          ...
          this.estimateSizeRecursive(obj[key], visited)
        );
      }, 0);
    } catch {
