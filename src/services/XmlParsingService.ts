...
export class XmlParsingService {
  ...
  public parseFindItemResponse(xmlResponse: string): ParsedEmail[] {
    try {
      ...
      let messages = this.getElementsByTagNameNS(
        xmlDoc,
        EWS_NAMESPACES.EWS_TYPES,
        "Message",
      );

      // Fallback: if no messages found with namespace, try namespace-agnostic search
      if (messages.length === 0) {
        ...
        for (let i = 0; i < allElements.length; i++) {
          ...
          if (
            el.localName === "Message" ||
            el.tagName === "Message" ||
            el.tagName === "t:Message"
          ) {
            found.push(el);
          }
        }
   ...
   */
  public parseConversationResponse(
    xmlResponse: string,
    ...
    try {
      ...
      for (let i = 0; i < messages.length; i++) {
        const message = messages[i];
        const threadMessage = this.parseThreadMessage(
          message,
          currentUserEmail,
        );
        if (threadMessage) {
  ...
  private parseEmailMessage(messageElement: Element): ParsedEmail | null {
    try {
      const email: ParsedEmail = {
        ...
        dateTimeSent:
          this.getElementTextContent(messageElement, "DateTimeSent") || "",
        conversationId:
          this.getElementAttribute(messageElement, "ConversationId", "Id") ||
          "",
        body: {
        ...
        from: {
          emailAddress: {
            address:
              this.getNestedElementText(messageElement, [
                "From",
                "Mailbox",
                "EmailAddress",
              ]) || "",
   ...
   */
  private parseThreadMessage(
    messageElement: Element,
    currentUserEmail: string,
  ): ParsedThreadMessage | null {
    try {
      const fromEmail =
        this.getNestedElementText(messageElement, [
          "From",
      ...
      const threadMessage: ParsedThreadMessage = {
        id: this.getElementAttribute(messageElement, "ItemId", "Id") || "",
        changeKey:
          this.getElementAttribute(messageElement, "ItemId", "ChangeKey") ||
          undefined,
        subject: this.getElementTextContent(messageElement, "Subject") || "",
        from: fromEmail,
        to: this.parseRecipientsList(messageElement, "ToRecipients").map(
          (r) => r.emailAddress.address,
        ),
        sentDate: dateTimeText ? new Date(dateTimeText) : new Date(),
        body: this.cleanBodyContent(
          this.getElementTextContent(messageElement, "Body") || "",
        ),
   ...
   */
  private parseRecipientsList(
    messageElement: Element,
    recipientsType: string,
  ): Array<{ emailAddress: { address: string } }> {
    try {
      const recipientsElement = this.getChildElementByTagName(
        messageElement,
        recipientsType,
      );
      ...
      const mailboxElements = this.getChildElementsByTagName(
        recipientsElement,
        "Mailbox",
      );
      ...
      for (let i = 0; i < mailboxElements.length; i++) {
        ...
        const emailAddress = this.getElementTextContent(
          mailbox,
          "EmailAddress",
        );
        if (emailAddress) {
   ...
   */
  private getChildElementByTagName(
    parent: Element,
    localName: string,
  ): Element | null {
    // Try with EWS types namespace first
    const namespacedElements = this.getElementsByTagNameNS(
      parent,
      EWS_NAMESPACES.EWS_TYPES,
      localName,
    );
   ...
   */
  private getChildElementsByTagName(
    parent: Element,
    localName: string,
  ): Element[] {
    // Try with EWS types namespace first
   ...
   */
  private getElementsByTagNameNS(
   ...
   */
  public validateEwsResponse(xmlResponse: string): {
    isValid: boolean;
    error?: string;
  } {
    try {
      ...
      const responseMessages = xmlDoc.getElementsByTagNameNS(
        EWS_NAMESPACES.EWS_MESSAGES,
        "ResponseMessages",
      )[0];
      if (responseMessages) {
        const errorResponse = responseMessages.querySelector(
          '[ResponseClass="Error"]',
        );
        if (errorResponse) {
          const responseCode =
            errorResponse.querySelector("ResponseCode")?.textContent ||
            "Unknown error";
          const messageText =
            errorResponse.querySelector("MessageText")?.textContent || "";
          return {
            isValid: false,
