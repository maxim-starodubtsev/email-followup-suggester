...
describe("ConfigurationService", () => {
  ...
  beforeEach(() => {
    ...
    (global as any).Office.context.mailbox.getUserIdentityTokenAsync = vi.fn();
  ...
  describe("Account Management", () => {
    it("should get available accounts", async () => {
      (
        global as any
      ).Office.context.mailbox.getUserIdentityTokenAsync.mockImplementation(
        (callback: any) => {
          callback({ status: "succeeded" });
        },
      );
      (global as any).Office.context.mailbox.userProfile.emailAddress =
        "user@example.com";
