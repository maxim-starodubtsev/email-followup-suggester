...
describe('BatchProcessor', () => {
    ...
    describe('Basic Batch Processing', () => {
        ...
        it('should use default batch size when not specified', async () => {
            ...
            const processor = jest.fn().mockImplementation(async (item: number) => item);
    ...
    describe('Integration Scenarios', () => {
        it('should handle complex processing scenarios', async () => {
            ...
            const processor = jest.fn().mockImplementation(async (email: EmailData) => {
                // Simulate processing time based on priority
                const delay = email.priority === 'high' ? 100 : email.priority === 'medium' ? 50 : 25;
            ...
            expect(progressUpdates[progressUpdates.length - 1]).toEqual({ current: 5, total: 5 });
        });
    });
});