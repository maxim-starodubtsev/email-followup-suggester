...
describe("LlmService", () => {
  ...
  let mockRetryService: vi.Mocked<RetryService>;

  beforeEach(() => {
    mockFetch = fetch as vi.MockedFunction<typeof fetch>;
    mockFetch.mockClear();
    ...
    // Mock retry service
    mockRetryService = {
      executeWithRetry: vi.fn().mockImplementation((operation) => operation()),
    } as any;
...
3. Let me follow up with the team and provide an update soon.`;
      ...
      expect(suggestions[2]).toBe(
        "Let me follow up with the team and provide an update soon.",
      );
    });
    ...
    it("should handle API errors", async () => {
      ...
      await expect(
        service.generateFollowupSuggestions("Test email"),
      ).rejects.toThrow(
        "Failed to generate followup suggestions: DIAL API request failed: 500 Internal Server Error - Server Error",
      );
    });
    ...
    it("should include context in prompt when provided", async () => {
      ...
      await service.generateFollowupSuggestions(
        "Test email",
        "Important context",
      );
  ...
  describe("analyzeTone", () => {
    ...
    it("should handle tone analysis errors", async () => {
      ...
      await expect(service.analyzeTone("Test email")).rejects.toThrow(
  ...
  describe("summarizeEmail", () => {
    it("should summarize email successfully", async () => {
      const mockResponse =
        "This email discusses project updates and timelines.";
      ...
      const summary = await service.summarizeEmail(
        "Long email content about project...",
      );
  ...
  describe("analyzeThread", () => {
    it("should analyze email thread successfully", async () => {
      const mockResponse =
        "The thread discusses project status with increasing urgency.";
  ...
  describe("API Configuration", () => {
    ...
    it("should throw error when API endpoint is not configured", async () => {
      ...
      await expect(
        invalidService.generateFollowupSuggestions("Test"),
